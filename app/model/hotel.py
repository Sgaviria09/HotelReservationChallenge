...
@dataclass
class Guest:
    REGULAR: str = field(default="regular", init=False, repr=False)
    VIP: str = field(default="vip", init=False, repr=False)
...
class Room:
    ...
    def book(self, reservation_id: str, check_in: date, check_out: date):
        # First verify all dates are available
        current = check_in
        ...
        # Then assign reservation_id to all dates
        current = check_in
        while current < check_out:
...
class Hotel:
    ...
    def add_room(self, number: int, type_: str, price_per_night: float):
        ...
        self.rooms[number] = Room(number=number, type_=type_, price_per_night=price_per_night)
    ...
    def add_guest(self, reservation_id: str, name: str, email: str, type_: str):
        ...
        if not reservation:
            reservation_not_found_error()
        reservation.add_guest(name=name, email=email, type_=type_)
